...
(20 points) [\textbf{Analysis}] Suppose that, instead of promoting an element from $L_{i-1}$ into $L_i$ based on a coin toss, we promote it with some probability $p$, $0 < p < 1$.
 
\begin{enumerate}[(a)]
\item (5 points) Show that, with this modification, the expected length of a search
path is at most $\frac{1}{p} log_{\frac{1}{p}}(n)$.
\item (5 points) What is the value of $p$ that minimizes the preceding expression?
\item (5 points) What is the expected height of the skiplist?
\item (5 points) What is the expected number of nodes in the skiplist?
\end{enumerate}
...
\textbf{Compilation Guidelines:} Before proceeding with submission, kindly verify that your code compiles utilizing the \textbf{C++17} standard.  MacOS and Linux users must compile the Raylib library from its source in order to successfully compile the program.  The instructions for compiling the raylib from the source can be accessed from \cite{ref3}.
...
Due date: Sunday, 11 March 2024, 9:00 AM.
